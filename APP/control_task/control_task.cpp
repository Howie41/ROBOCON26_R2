...
void Xbox_Data_Process() {
    if (ABS(control_xbox_cmd.joyLVert - 32767) > 2300) chassis_cmd.linear_x_ = -(int)(control_xbox_cmd.joyLVert - 32767) / 32767.0f * MAX_VELOCITY;
    else chassis_cmd.linear_x_ = 0.0f;

    if (ABS(control_xbox_cmd.joyLHori - 32767) > 2300) chassis_cmd.linear_y_ = (int)(control_xbox_cmd.joyLHori - 32767) / 32767.0f * MAX_VELOCITY;
    else chassis_cmd.linear_y_ = 0.0f;

    if (ABS(control_xbox_cmd.joyRHori - 32767) > 2300) chassis_cmd.omega_ = -(int)(control_xbox_cmd.joyRHori - 32767) / 32767.0f * MAX_ROTATION_VELOCITY;
    else chassis_cmd.omega_ = 0.0f;
}

void Lift_Data_Process() {
    if (control_xbox_cmd.btnY && !xbox_y_last) xbox_y_press_tick = xTaskGetTickCount();
    
    lift_cmd.request_high = false;
    if (!control_xbox_cmd.btnY && xbox_y_last) {
        if ((xTaskGetTickCount() - xbox_y_press_tick) < kLiftTapTimeout) lift_cmd.request_high = true;
    }

    xbox_y_last = control_xbox_cmd.btnY;
    if (control_xbox_cmd.btnA && !xbox_a_last) xbox_a_press_tick = xTaskGetTickCount();

    lift_cmd.request_low = false;
    if (!control_xbox_cmd.btnA && xbox_a_last) {
        if ((xTaskGetTickCount() - xbox_a_press_tick) < kLiftTapTimeout) lift_cmd.request_low = true;
    }
    ...
    if (ABS(control_xbox_cmd.joyRVert - 32767) > 2000) lift_cmd.lift_2006_input = (int)(control_xbox_cmd.joyRVert - 32767) / 32767.0f * MAX_LIFT_VELOCITY;
    else lift_cmd.lift_2006_input = 0.0f;
}
...
static void Arm_Data_Process() {
    if (control_xbox_cmd.btnX && !xbox_x_last)  xbox_x_press_tick = xTaskGetTickCount();

    arm_cmd.update = false;
    if (!control_xbox_cmd.btnX && xbox_x_last) {
        if ((xTaskGetTickCount() - xbox_x_press_tick) < kLiftTapTimeout) {arm_cmd.update = true;
    }
    xbox_x_last = control_xbox_cmd.btnX;

    if (control_xbox_cmd.btnB && !xbox_b_last) xbox_b_press_tick = xTaskGetTickCount();

    arm_cmd.fetch = false;
    if (!control_xbox_cmd.btnB && xbox_b_last) {
        if ((xTaskGetTickCount() - xbox_b_press_tick) < kLiftTapTimeout) arm_cmd.fetch = true;
    }
...
static bool consumeTriggerRisingEdge(uint16_t current_value, uint16_t threshold, bool *last_state) {
...
static LtAction consumeLtAction(uint16_t current_value) {
    ...
    if (pressed && !xbox_lt_pressed_last) xbox_lt_press_tick = xTaskGetTickCount();
    ...
    if (!pressed && xbox_lt_pressed_last) {
        ...
        action = (hold_ticks >= kLtLongPressThreshold) ? LtAction::ReturnToCenter : LtAction::GoToEdge;
    }
...
void controlTask(void *argument) {
    ...
    for (;;) {
        if (control_xbox_sub.TryGet(&control_xbox_cmd)) {
            ...
            if (!nav_control::auto_enabled) {
                const bool stair_y_pressed = consumeButtonRisingEdge(control_xbox_cmd.btnY, &xbox_y_last_for_stair);
                const bool stair_a_pressed = consumeButtonRisingEdge(control_xbox_cmd.btnA, &xbox_a_last_for_stair);
                const LtAction lt_action = consumeLtAction(control_xbox_cmd.trigLT);
                const bool climb_r1_pressed = consumeTriggerRisingEdge(control_xbox_cmd.trigRT, kR1TriggerThreshold, &xbox_rt_last);
                ...
                if (stairWaypointArmed() && manualActionReady()) {
                    ...
                    if (lt_action == LtAction::ReturnToCenter && stairWaypointLevel() > 0U) {
                ...
                if (consumeButtonRisingEdge(control_xbox_cmd.btnLB, &xbox_lb_last) && manualActionReady()) {
                    chassis_action::turn_left_90_deg();
                    vTaskDelayUntil(&currentTime, 5);
                    continue;
                }

                if (consumeButtonRisingEdge(control_xbox_cmd.btnRB, &xbox_rb_last) && manualActionReady()) {
