...
public:
    ...
    float getRawCurrentTorque(void) const { return raw_torque_; }
      ...
      // offline 检测
  void setOfflineDeadline(const uint32_t offline_deadline) { offline_wd_.setTimeout(offline_deadline); }
  void setOfflineDebounce(const uint32_t offline_debounce) { offline_wd_.setDebounce(offline_debounce); }
