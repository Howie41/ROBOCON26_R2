...
volatile float g_nav_brake_safety_scale = 10.00f;
...
volatile float g_nav_pid_max_xy_speed_mps = 0.55f;
...
constexpr TickType_t kPositionTimeoutTicks = pdMS_TO_TICKS(300);
...
float calcBlendFactor(float dist_mm) {
  ...
  float t = clampf((dist_mm - g_nav_arrive_dist_mm) / denom, 0.0f, 1.0f);
  ...
  for (;;) {
    ...
    if (nav_control::auto_enabled) {
      ...
      if (nav_control::high_mode_active) {
        ...
        if (high_cmd.forward_speed < -1000.0f)
          high_cmd.forward_speed = -1000.0f;
